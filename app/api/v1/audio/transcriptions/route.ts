...
async function forwardAudio(req: NextRequest, endpoint: string) {
  ...
  const upstream = await fetch(`${(p.apiBaseUrl ?? "").replace(/\/$/, "")}/${endpoint}`, {
    method: "POST",
...
export async function POST(req: NextRequest) {
  return forwardAudio(req, "audio/transcriptions");
}
